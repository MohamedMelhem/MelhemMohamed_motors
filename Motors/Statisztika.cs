...
namespace Motors
{
	internal class Statisztika
	{
		...
		public Motor Oldest()
		{
			...
			for (int i = 0; i < motors.Count; i++)
			{
				if (motors[i].ReleseYear < oldestMotor.ReleseYear)
				{
		...
		public void PrintMotors()
		{
			...
			{
				Console.WriteLine($"{motor.Brand} {motor.Name} ({motor.ReleseYear}) - Performance: {motor.Performance}, Price: {motor.PriceInEur} EUR");
			}
		}
